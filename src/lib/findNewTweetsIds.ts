...
const findNewTweetsIds = (tweetList: TweetType[], listOfTweetIds: number[]): number[] => {
    // empty array where I add every new tweet id
    let newTweetsIdsList: number[] = [];
    // for each tweet in the full list of tweets
    tweetList.forEach(tweet => {
        // I look for an id in "listOfTweetIds" matching the id value of the given tweet
        ...
        if(tweetIdAlreadyPresent === undefined) {
            newTweetsIdsList.push(tweet.id);
        }
    });
